//监控区域模块制作 ： 利用原生JS立刻执行函数
(function(){
    //我们将这个文件在所有代码最底端引入，这样就不用写入口函数(用来等待我们DOM都加载完再执行)
    // $(function(){})


    $(".monitor .tabs").on("click","a",function(){
        //此事件是绑定在tabs类的div，触发对象是机器内部的a

        //谁被点击谁就高亮显示，其余弟兄移除高亮（if applicable）
        $(this).addClass('active').siblings().removeClass('active');

        //获取 被点击的a 的索引号
        let index = $(this).index();

        //选取对应索引号的content，让其显示:a水量必须等于content数量
...
(function(){
    ...
    var option = {
        ...
        series: [
            // 第一个图，没有第二个 如果要在添加第二个对象到series数组里 。js对象里面每一个数据都要用逗号隔开
            {
                name: '点位统计',
                // 类型
                type: 'pie',
                // 1:里面圆圈半径 2:外面圆形的半径 像素或者百分比（根据装下它的（父）DOM容器宽度来做）
                radius: ["10%","70%"],
                //50% 50% 就是水平居中x 垂直居中y
                center: ['50%', '50%'],
                //raidus半径模式 area面积模式
